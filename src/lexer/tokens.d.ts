/** Apache Velocity: Lexer Tokens | OWNER: vela | STATUS: READY */
import { TokenType, Lexer } from 'chevrotain';
export declare const AnyTextFragment: TokenType;
export declare const StringLiteral: TokenType;
export declare const NumberLiteral: TokenType;
export declare const BooleanLiteral: TokenType;
export declare const NullLiteral: TokenType;
export declare const Identifier: TokenType;
export declare const DollarRef: TokenType;
export declare const QuietRef: TokenType;
export declare const InKeyword: TokenType;
export declare const InterpStart: TokenType;
export declare const LCurly: TokenType;
export declare const RCurly: TokenType;
export declare const LParen: TokenType;
export declare const RParen: TokenType;
export declare const LBracket: TokenType;
export declare const RBracket: TokenType;
export declare const Dot: TokenType;
export declare const Comma: TokenType;
export declare const Colon: TokenType;
export declare const Semicolon: TokenType;
export declare const Assign: TokenType;
export declare const Plus: TokenType;
export declare const Minus: TokenType;
export declare const Star: TokenType;
export declare const Slash: TokenType;
export declare const Mod: TokenType;
export declare const Question: TokenType;
export declare const Not: TokenType;
export declare const And: TokenType;
export declare const Or: TokenType;
export declare const Eq: TokenType;
export declare const Ne: TokenType;
export declare const Lt: TokenType;
export declare const Le: TokenType;
export declare const Gt: TokenType;
export declare const Ge: TokenType;
export declare const Range: TokenType;
export declare const EscapedDirective: TokenType;
export declare const Hash: TokenType;
export declare const IfDirective: TokenType;
export declare const ElseIfDirective: TokenType;
export declare const ElseDirective: TokenType;
export declare const SetDirective: TokenType;
export declare const ForEachDirective: TokenType;
export declare const BreakDirective: TokenType;
export declare const StopDirective: TokenType;
export declare const MacroDirective: TokenType;
export declare const EvaluateDirective: TokenType;
export declare const ParseDirective: TokenType;
export declare const IncludeDirective: TokenType;
export declare const EndDirective: TokenType;
export declare const LineComment: TokenType;
export declare const BlockComment: TokenType;
export declare const TemplateText: TokenType;
export declare const Whitespace: TokenType;
export declare const Newline: TokenType;
export declare const allTokens: TokenType[];
export declare const createLexer: () => Lexer;
