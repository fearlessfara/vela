...
async function testStreamSupport() {
    ...
    const result3 = await engine.evaluateReader({ message: 'Loaded asynchronously!' }, asyncTemplateLoader);
    ...
    let combinedOutput = '';
    const result4 = await engine.evaluateReader({ value: 'Combined test' }, async () => 'Reader template: $value', 'combined-test', (chunk) => { combinedOutput += chunk; });
